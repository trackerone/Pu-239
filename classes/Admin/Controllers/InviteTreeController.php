...
final class InviteTreeController
{
    ...
    public function __invoke(array $meta = []): void
    {
        ...
        try {
            // LEGACY BODY START (from admin/invite_tree.php)
            global $container;
            ...
            $inviteSql = "SELECT u.id, u.username, u.uploaded, u.downloaded, u.email, i.status FROM users AS u LEFT JOIN invite_codes AS i ON u.id = i.receiver WHERE u.invitedby = :id AND u.join_type = :join_type ORDER BY u.registered";
    ...
    <table>
        <tr>
            <td>';
                $query = $db->fetchAll($inviteSql, ['id' => $id, 'join_type' => 'invite']);
                if (empty($query)) {
                ...
                <table class="table table-bordered table-striped">
                    ...
                    foreach ($query as $arr_invited) {
                        ...
                        if (isset($_GET['deeper']) || isset($_GET['really_deep'])) {
                            $query2 = $db->fetchAll($inviteSql, ['id' => (int) $arr_invited['id'], 'join_type' => 'invite']);
                    ...
                    <tr>
                            ...
                            <div>
                                ...
                                foreach ($query2 as $arr_invited_deeper) {
                                    if (isset($_GET['really_deep'])) {
                                        $query3 = $db->fetchAll($inviteSql, ['id' => (int) $arr_invited_deeper['id'], 'join_type' => 'invite']);
                                    ...
                                    <tr>
                                        <td colspan="6"><span class="has-text-weight-bold">' . htmlsafechars($arr_invited_deeper['username']) . (substr($arr_invited_deeper['username'], -1) === 's' ? '\'' : '\'s') . ' Invites:</span>
                                            ...
                                            foreach ($query3 as $arr_invited_really_deep) {
                                                $deeper .= '
                                                    <tr>
                                                        <td>' . ($arr_invited_really_deep['status'] === 'Pending' ? htmlsafechars($arr_invited_really_deep['username']) : format_username((int) $arr_invited_really_deep['id'])) . '</td>
                                    ...
                                    $deeper .= '
                                    <tr>
                                        <td>' . ($arr_invited_deeper['status'] === 'Pending' ? htmlsafechars($arr_invited_deeper['username']) : format_username((int) $arr_invited_deeper['id'])) . '</td>
                        ...
                        $HTMLOUT .= '
                    <tr>
                        <td>' . ($arr_invited['status'] === 'Pending' ? htmlsafechars($arr_invited['username']) : format_username((int) $arr_invited['id'])) . '</td>
                ...
                $id = '';
                $search = isset($_GET['search']) ? strip_tags(trim((string) $_GET['search'])) : '';
                $class = isset($_GET['class']) ? $_GET['class'] : '-';
                $letter = '';
                $q = '';
                if ($class == '-' || !ctype_digit((string) $class)) {
                    $class = '';
                }
                ...
                if ($search !== '' || $class !== '') {
                    ...
                    $conditions[] = 'username LIKE :letter';
        ...
        <form method="get" action="staffpanel.php?tool=invite_tree&amp;search=1&amp;" accept-charset="utf-8">
            <div class="has-text-centered margin20">
                ...
                for ($i = 0;; ++$i) {
                    if ($c = get_user_class_name((int) $i)) {
                        $HTMLOUT .= '
                    <option value="' . $i . '" ' . (ctype_digit((string) $class) && $class == $i ? 'selected' : '') . '>' . $c . '</option>';
                    } else {
                        break;
                    }
                }
            ...
            <nav class="pagination is-centered is-marginless is-small" role="navigation" aria-label="pagination">
                ...
                $countValue = $db->fetchValue('SELECT COUNT(id) FROM users WHERE ' . $where, $params);
                ...
                if ($count > 0) {
                    $rows = $db->fetchAll(
                        'SELECT users.*, countries.name AS country_name, countries.flagpic AS country_flagpic FROM users FORCE INDEX ( username ) LEFT JOIN countries ON country = countries.id WHERE ' . $where . ' ORDER BY username ' . $LIMIT,
                        $params,
                    );
                    $heading = '
            <tr>
                ...
                <td>
                    <img src="' . $imagesBaseurl . 'flag/' . $s($row['country_flagpic']) . '" alt="' . htmlsafechars($row['country_name']) . '" title="' . htmlsafechars($row['country_name']) . '" class="tooltipper">
                </td>' : '
                <td>---</td>';
                        $body .= '
            <tr>
            ...
            error_log('Admin controller error (invite_tree): ' . $e->getMessage());
