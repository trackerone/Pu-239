...
final class StaffboxHandler
{
    ...
    public function handle(array $meta = []): void
    {
        // AUTO_CONVERT_ATTEMPTED: 2025-10-22T03:59:59Z via handler-convert offset=350 batch=5
        try {
            $target = dirname(__DIR__, 4) . '/public/staffbox.php';
            if (!is_file($target)) {
            ...
            // TODO(2025): extract legacy block from _quarantine/rebroken/public/staffbox.php (incomplete queries: $db->run('); markers)
            ob_start();
            try {
                require $target;
