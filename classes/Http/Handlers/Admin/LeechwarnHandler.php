...
final class LeechwarnHandler
{
    ...
    public function handle(array $meta = []): void
    {
        ...
        try {
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                // TODO(2025): add CSRF verification for leechwarn bulk actions
                $r = isset($_POST['ref']) ? (string) $_POST['ref'] : $thisUrl;
                ...
                if ($act === 'unwarn') {
                    ...
                    if (!has_access((int) ($CURUSER['class'] ?? 0), UC_SYSOP, 'coder')) {
            ...
            switch ($do) {
                case 'disabled':
                  ...
                  ORDER BY last_access DESC";
                    ...
                    $link = '<a href="' . $base . '/staffpanel.php?tool=leechwarn&amp;action=leechwarn&amp;do=warned">' . _('Leech warned users') . '</a>';
                    break;
                ...
                default:
                  ...
                  ORDER BY last_access DESC, leechwarn DESC";
                    ...
                    $link = '<a href="' . $base . '/staffpanel.php?tool=leechwarn&amp;action=leechwarn&amp;do=disabled">' . _('Disabled users') . '</a>';
                    break;
            }
    ...
    <form action='{$self}?tool=leechwarn&amp;action=leechwarn' method='post' enctype='multipart/form-data' accept-charset='utf-8'>";
                ...
                foreach ($rows as $a) {
                    ...
                    if ($do === 'leechwarn') {
                        $leechWarnAt = (int) $a['leechwarn'];
                        $expiresIn = mkprettytime($leechWarnAt - TIME_NOW);
                        $tip = _('Leech Warned for: ') . htmlsafechars((string) ($a['warn_reason'] ?? '')) . '<br>'
                            . _('Warned till ') . get_date($leechWarnAt, 'DATE', 1) . ' - ' . $expiresIn;
                    } else {
                        $tip = _('Disabled for ') . htmlsafechars((string) ($a['disable_reason'] ?? ''));
                    }
                ...
                $HTMLOUT .= "<select name='action'>";
                $HTMLOUT .= "<option value='unwarn'>" . _('Unwarn') . '</option>';
                $HTMLOUT .= "<option value='disable'>" . _('Disable') . '</option>';
                $HTMLOUT .= "<option value='delete' "
                    . (!has_access((int) ($CURUSER['class'] ?? 0), UC_SYSOP, 'coder') ? 'disabled' : '')
                    . '> ' . _('Delete') . '</option>';
                $HTMLOUT .= '</select>&raquo;';
                $HTMLOUT .= "<input type='submit' value='" . _('Apply') . "' class='button is-small'>";
                $HTMLOUT .= "<input type='hidden' value='{$requestUri}' name='ref'>";
                $HTMLOUT .= '</div></form>';
            }

            $breadcrumbs = [
                "<a href='{$baseurl}/staffpanel.php'>" . _('Staff Panel') . '</a>',
                "<a href='{$self}'>" . $sanitize($title) . '</a>',
            ];
