...
$breadcrumbs = [
    ...
    '<a href="' . $config->get('paths.baseurl') . '/forums.php?action=subscriptions">' . _('Subscriptions') . '</a>',
];
if ($count === 0) {
    $HTMLOUT .= main_div("
...
while ($topic_arr = mysqli_fetch_assoc($res)) {
    ...
    $first_post_res = sql_query('SELECT p.added, p.icon, p.body, p.user_id, p.anonymous, u.id, u.username, u.class, u.donor, u.warned, u.status, u.chatpost, u.leechwarn, u.pirate, u.king FROM posts AS p LEFT JOIN users AS u ON p.user_id=u.id WHERE ' . ($CURUSER['class'] < UC_STAFF ? 'p.status = \'ok\' AND' : ($CURUSER['class'] < $config->get('forum_config.min_delete_view_class') ? 'p.status != \'deleted\' AND' : '')) . ' topic_id=' . sqlesc($topic_id) . ' ORDER BY id DESC LIMIT 1') or sqlerr(__FILE__, __LINE__);
    ...
    switch (true) {
        ...
        case $total_pages > 11:
            $multi_pages = ' <span style="font-size: xx-small;"> <img src="' . $config->get('paths.images_baseurl') . 'forums/multipage.gif" class="icon tooltipper" alt="+" title="+">';
            for ($i = 1; $i < 5; ++$i) {
                $multi_pages .= ' <a class="is-link" href="' . $config->get('paths.baseurl') . '/forums.php?action=view_topic&amp;topic_id=' . $topic_id . '&amp;page=' . $i . '">' . $i . '</a>';
            }
            $multi_pages .= ' ... ';
    ...
    $new = ($topic_arr['added'] > (TIME_NOW - $config->get('forum_config.readpost_expiry'))) ? (!$last_unread_post_arr || $lppostid > $last_unread_post_arr[0]) : 0;
    $topicpic = ($posts < 30 ? ($locked ? ($new ? 'lockednew' : 'locked') : ($new ? 'topicnew' : 'topic')) : ($locked ? ($new ? 'lockednew' : 'locked') : ($new ? 'hot_topic_new' : 'hot_topic')));
    $topic_name = ($sticky ? '<img src="' . $config->get('paths.images_baseurl') . 'forums/pinned2.gif" class="icon tooltipper" alt="' . _('Pinned') . '" title="' . _('Pinned') . '"> ' : ' ') . ($topicpoll ? '<img src="' . $config->get('paths.images_baseurl') . 'forums/poll.gif" class="icon tooltipper" alt="' . _('Poll') . '" title="' . _('Poll') . '"> ' : ' ') . ' <a class="is-link" href="' . $config->get('paths.baseurl') . '/forums.php?action=view_topic&amp;topic_id=' . $topic_id . '">' . format_comment($topic_arr['topic_name']) . '</a> ' . $multi_pages;
    $body .= '<tr>
		<td><img src="' . $config->get('paths.images_baseurl') . 'forums/' . $topicpic . '.gif" class="icon tooltipper" alt="' . _('Topic') . '" title="' . _('Topic') . '"></td>
		<td>' . $icon . '</td>
		<td>
	    	' . $topic_name . $first_post_text . ($new ? ' <img src="' . $config->get('paths.images_baseurl') . 'forums/new.gif" class="icon tooltipper" alt="' . _('New post in topic') . '!" title="' . _('New post in topic') . '!">' : '') . '</td>
    		' . $rpic . '
		...
		<td><a class="is-link" href="' . $config->get('paths.baseurl') . '/forums.php?action=view_topic&amp;topic_id=' . $topic_id . '&amp;page=p' . $last_post_id . '#' . $last_post_id . '">
		<img src="' . $config->get('paths.images_baseurl') . 'forums/last_post.gif" class="icon tooltipper" alt="Last post" title="Last post"></a></td>
		<td><input type="checkbox" name="remove[]" value="' . (int) $topic_arr['subscribed_id'] . '"></td>
		</tr>';
}

$HTMLOUT .= ($count > $perpage ? $menu_top : '') . '<form action="' . $config->get('paths.baseurl') . '/forums.php?action=delete_subscription" method="post" name="checkme" accept-charset="utf-8">';
$heading = '
		<tr>
		...
		<th>' . _('Views') . '</th>
		<th>' . _('Last Post') . '</th>
		<th><img src="' . $config->get('paths.images_baseurl') . 'forums/last_post.gif" class="icon tooltipper" alt="Last post" title="Last post"></th>
