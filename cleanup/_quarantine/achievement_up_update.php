...
$db = $container->get(Database::class);, $site_config;
    ...
    $rows = $db->fetchAll('SELECT u.id, u.numuploads, a.ul FROM users AS u LEFT JOIN usersachiev AS a ON u.id = a.userid WHERE u.status = 0 AND u.numuploads >= 1');
    ...
    if (mysqli_num_rows($res) > 0) {
        ...
        echo $text . "\n";
        if ($data['clean_log']) {
            write_log('Achievements Cleanup: Uploader Completed. Uploader Achievements awarded to - ' . $count . ' Member(s).' . $text);
        }
