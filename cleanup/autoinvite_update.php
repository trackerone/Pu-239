...
$db = $container->get(Database::class);, $site_config;
    ...
    $dt = TIME_NOW;
    $joined = ($dt - 86400 * 90);
    $rows = $db->fetchAll('SELECT id, uploaded, invites, downloaded, modcomment FROM users WHERE invites = 1 AND class = ' . UC_MIN . " AND uploaded / downloaded <= $ratiocheck AND status = 0 AND registered < $joined") or sqlerr(__FILE__, __LINE__);
