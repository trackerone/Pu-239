...
if ($count === 0) {
    ...
    foreach ($rows as $arr2) {
        ...
        if (!has_access((int) $arr2['class'], UC_ADMINISTRATOR, 'coder') && (int) $arr2['id'] !== (int) $CURUSER['id']) {
            ...
            $body .= '</td>
            <td>' . _fe('Until {0} ({1}) to go.', get_date($personal_freeleech, 'DATE'), mkprettytime($personal_freeleech - $dt)) . "</td>
            <td><a href='{$site_config['paths']['baseurl']}/staffpanel.php?tool=freeusers&amp;remove=" . (int) $arr2['id'] . "' onclick=\"return confirm('" . _('Are you sure you want to remove this users Freeleech Status?') . "')\">" . _('Remove') . '</a></td>
        </tr>';
        }
    }
