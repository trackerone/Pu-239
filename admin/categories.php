...
function move_cat($params)
{
    ...
    $results = $sql = "UPDATE torrents SET /* columns */ WHERE category = :category"
$this->db->perform($sql, array_merge($set, ['category' => $params['id']]));
...
function add_cat($params)
{
    ...
    $fluent = $container->get(Database::class);
    $insert = $sql = "INSERT INTO categories (/* columns */) VALUES (/* values */)"
...
function delete_cat($params)
{
    ...
    $results = $sql = "DELETE FROM categories WHERE id = :id"
$this->db->perform($sql, ['id' => $params['id']]);
...
function edit_cat($params)
{
    ...
    $fluent = $container->get(Database::class);
    $update = $sql = "UPDATE categories SET /* columns */ WHERE id = :id"
$this->db->perform($sql, array_merge($set, ['id' => $params['id']]));
...
$this->db->perform($sql, array_merge($set, ['id' => $cat['id']]));
    }
