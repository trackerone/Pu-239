...
$valid = [
    ...
    'tickets' => [
        'minclass' => $config->get('allowed.play'),
...
switch (true) {
    ...
    default:
        $HTMLOUT = "
                    <h1 class='has-text-centered'>" . _fe('{0} Lottery', $config->get('site.name')) . '</h1>';
       ...
       <div class='top10'>" . ($user['class'] >= $valid['viewtickets']['minclass'] ? "<a href='{$config->get('paths.baseurl')}/lottery.php?action=viewtickets' class='button is-small margin10'>" . _('View bought tickets') . '</a>' : '') . "<a href='{$config->get('paths.baseurl')}/lottery.php?action=tickets' class='button is-small margin10'>" . _('Buy tickets') . '</a></div>', 'bottom20 has-text-centered');
        }
        ...
        <p>' . ($user['class'] >= $valid['config']['minclass'] ? "<a href='{$config->get('paths.baseurl')}/lottery.php?action=config' class='button is-small margin10'>" . _('Lottery Configuration') . '</a>' : _('Nothing Configured Atm Sorry')) . '</p>', 'top20');
        } else {
            $HTMLOUT .= main_div("
                        <div class='padding20 has-text-centered'>
                            ...
                            <a href='{$config->get('paths.baseurl')}/lottery.php?action=config' class='button is-small'>" . _('Lottery Configuration') . '</a>' : '
